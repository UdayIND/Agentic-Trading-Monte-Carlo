...
| IB-12 | 2026-07-03 | NVDA order session | python missing on this machine — tools/guard.py + regime.py + scheduled runs cannot execute; install Python 3 and verify `python -m unittest discover -s tools/tests` green | deterministic gates actually runnable; removes manual-math fallback | proposed (user: install python.org 3.12+ or `winget install Python.Python.3.12`) |
